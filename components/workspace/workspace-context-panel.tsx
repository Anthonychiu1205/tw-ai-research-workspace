  ...
  return (
    <div className="space-y-3" data-testid="workspace-context-panel">
      <Panel className="space-y-2 text-xs">
        <SectionHeading title={t("runtime.contextSection")} subtitle={artifact.title} />
      ...
      {(artifact.type === "backtest_summary" || artifact.kind === "backtest_summary") && (
        <Panel className="text-xs text-muted-foreground">Backtest summary is mock-only preview with no execution path.</Panel>
      )}
      ...
      {![
        ...
        <Panel className="space-y-2">
          <SectionHeading title={t("artifacts.title")} subtitle={t("common.open")} />
          <ArtifactJsonViewer data={data} />
        </Panel>
      ) : null}
    </div>
  );
