  ...
  return (
    <div className="space-y-2 rounded-lg border border-border bg-muted/40 p-3 text-xs" data-testid="scenario-result-panel">
      <div className="flex items-center gap-2">
        ...
        <StatusBadge tone={result.status === "succeeded" ? "success" : "danger"}>
          {result.status === "succeeded" ? t("scenarios.success") : result.status}
        </StatusBadge>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {result.createdArtifactIds.map((artifactId) => (
          <Button key={artifactId} type="button" size="sm" variant="outline" onClick={() => onOpenArtifact?.(artifactId)}>
            {t("common.openArtifact")} {artifactId.slice(0, 8)}
          </Button>
        ))}
      </div>
    </div>
